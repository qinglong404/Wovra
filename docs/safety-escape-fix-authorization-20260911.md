...
- git push：曾位于 `_DENIED_PATTERNS` 黑名单，2026-09-11 用户拍板改走确认门
  （执行前 y/N 授权一次），不再硬拦

## 两平台等价修复（2026-09-11 晚，Windows 实测驱动）

Linux 侧 321 全绿，但同一套测试在 Windows 上 **3 红 + 1 挂死**——
根因两条，均与判定逻辑无关、纯平台通道缺失：

1. **守卫只认 POSIX 路径**：`_outside_absolute_paths` 只有 `/` 开头的
   规则，Windows 命令里的 `C:\...` / `C:/...` / UNC 完全不被识别 →
   探针 9 条 shell 越界用例全部漏拦（cat/rm/重定向拿走界外金丝雀）；
   其中 `ln` 用例还会把界内链接重指向界外，连累后续反向对照假红。
   修复：新增 `_WIN_ABS_PATH` 通道——判定与其它通道一致（解析后落在
   工作区内放行，界外走授权门）。Linux 上该通道天然惰性（`C:\x` 解析
   为 cwd 下相对路径 → 放行），无回归风险。
2. **测试自身的平台假设**：授权清单落盘是 JSON，Windows 路径的反斜杠
   被转义（`C:\\Users\\…`），原始文本子串断言假红——改为解析 JSON 断
   言语义内容，平台无关。

附带修掉一个测试挂死：安全探针的假工具层会真实执行 `find /`（Windows
PATH 含 GNU find → 全盘扫描）且用 PIPE 收输出被孙进程攥住。改用与
`run_command` 同款纪律：输出走临时文件 + 超时杀进程树。

结果：Windows 与 Linux 同为 **321 passed**。
