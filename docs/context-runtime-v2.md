...
- 全量追加；**下一次请求的上下文体量估算**（摘要 + 未压缩轮次的
  全部事件）达 **80% × 窗口（1M → 80 万 token）** 时，
  一次 LLM 调用把较早轮次压成摘要 + 保留最近 2-3 轮原文
- 阈值比例（`WOVRA_COMPRESS_THRESHOLD`）与绝对值均可配置
- **[修订 2026-09-05] 水位口径**：曾误用"累计计费输入"（每步重复计
  整段 prompt）为水位——实测虚增 19 倍（会话 3c87e1：真实上下文
  3 万 tok、账本 58 万，窗口远未吃紧即被提前压缩）。已改为窗口
  占用口径（`_baseline_context_estimate`），与 managed 的窗口保底
  同口径；计费口径仅作成本记录
...
* 步数失控的单轮预算（V3）
* 关键词相关性增强（V3 可选）
* **相关性自动升档（auto-promote，V3 可选，实测数据决定）**：
  当检测到当前输入与某个已降档轮次（档 2/档 3）高度相关时，
  Runtime 自动将其升回档 1（浓缩视图），省去模型一次 expand 工具
  调用（≈ 一步推理 + 10-60s 延迟）。漏取可自愈（模型自己 expand），
  错取是主动污染 → 默认关闭，**由实测数据决定是否启用**：
  实验期间记录 expand_history 的调用频率与触发时机——
  若模型频繁在降档轮次上展开则启用，若几乎不调则档 1 已够

---

## 12. 实验观察项（正式对照测试期间收集）

1. **expand_history 调用频率与时机**：决定 auto-promote 是否启用（见第 11 节）
2. **精修索引质量抽检**：随机抽取若干轮，人工比对 RefinedIndex 是否
   丢失关键结论（截断模板覆盖不足的信号）
3. **档位降档实际发生点**：多少轮后预算真实吃紧、哪一档先被触发
4. **CHR 曲线**：两种模式的缓存命中率随轮数的变化
   （managed 的批量换入是否真的保护了缓存）
5. **Historical Recall 测试**（第 7.3 节）：埋点事实在几十轮后的找回成功率
