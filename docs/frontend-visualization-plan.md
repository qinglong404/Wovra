...
| 域注册表 | agent 卡片（id/名称/状态点/文件域/收件箱），分裂判据摘录；账本显示「名下 N 轮（R21–R30）/ 步 / 消费」（§56：轮数统一按**总轮 R 号**，号即展开入口；消费按调用方实记，整理开销单列） | registry + 近轮 split_assessment + views.agent_ledger |
