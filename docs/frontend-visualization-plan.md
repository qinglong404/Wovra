...
> 状态：**意图存档 + 第一阶段已实现**（`wovra serve` 只读 API + `webui/`
...
对话流（C 线，默认视图）、轮×块时间线、状态账本、阶段/工作项计划、agent
...
| 域注册表 | agent 卡片（id/名称/状态点/文件域/收件箱），分裂判据摘录；账本显示「名下 N 轮（R21–R30）/ 步 / 消费」，用量页另有**按阶段**的轮账表（§58：分裂前整段记、不归 agent；分裂后按落点；总轮 = Σ各阶段） | registry + 近轮 split_assessment + views.agent_ledger / stage_spans |
...
| `GET /api/sessions/{id}/tree` | `{root,files:[{p,s}],truncated,error?}` | 项目文件树（工作区扁平相对路径；排除 .git/node_modules/虚拟环境/.shots 等重型目录，上限 4000 条）——「项目」页渲染为可折叠目录树，点击复制相对路径，Ctrl/⌘ 多选批量复制；同页并排渲染 agent 职责划分（注册表）与文件操作状态（blocks.ops 聚合） |
...
## 5. 阶段切分（对齐 todo 的两层制：阶段 = 可验收增量、工作项 = 阶段内拆解）
