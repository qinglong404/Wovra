...
class Task:
    ...
    file_map_sig: str = ""
    # **公共文件**（2026-09-18 用户口径）：结构树里没被任何节点认领的活性文件
    # 不再让整批分裂作废——它们进这里当"公共文件"，**所有 agent 都有全权**；
    # 谁**第一次写/改**它，所有权就归谁（此后别人只能读）。
    # 于是"模型这次没写全"只是**欠一分**，不是**全盘作废**：每个环节都不卡死，
    # 后面环节能修（下一批分裂仍可把它认给某个节点）。
    public_files: list = field(default_factory=list)
    created_at: str = ""
    updated_at: str = ""
