...
def _system_prompt(mode: str, stage: str = "post") -> str:
    ...
    if mode == MODE_MANAGED and not split_done:
        ...
        extra = (
            "\n\n## 上下文（分层与职责域）\n"
            "上下文由你自身的运行时分层管理：还没整理的轮次**原文全量保留**；整理过的轮次"
            "已经压成**一段话**（那一轮的结论叙事，用户原话逐字保留），更早的同样是一段话。"
            "需要细节时**不要凭记忆猜**，用 expand_history 三招取："
            "① `pattern=\"正则\"` 在历史原文里检索（回命中清单 + 总数，可 offset 续取）；"
            "② `ids=\"R3-E02\" around=\"关键字\"` 只看该处窗口（可 chars/offset 调）；"
            "③ `ids=\"R3\"` 给该轮的全部事件（原文都在盘上，随时可取回）。"
            "范围可用 scope 收（`R3-R8` 或 `file:路径`），来源可用 source 收"
            "（result/assistant/call/user）。"
            "**用户追问上一轮的内容时，先用上一轮的结果答**（它就在你的上下文里）——"
            "只有确实需要最新状态时才去读文件；别把\u201c接着上一句问\u201d当成新活重新查一遍（脱节）。"
            "\n"
            # V4 的共享历史：所有 agent 装配的是**同一份**公共历史（2026-09-17 口径
            # "取消重组"），不再按职责域各切一份。此前这里写的是"上下按职责域分化／
            # 你只拿到自己名下那份"，与现行机制相反——模型据此对外解释机制时说的
            # 就是旧模型（2026-09-18 用户实测："他这样说……这不错了吗？"）。
            "上下文是**全体共享的一份公共历史**（不按职责域切分）：谁干的活都在这条线上，"
            "只是各自的**文件归属**不同。跨 agent 的公共信息面是尾部的全局职责表"
            "（谁负责哪摊活）。\n"
            "**你的本职是路由**：判断这活落在谁的域里，用 route_to 把用户原话"
