...
SPECS: tuple[Spec, ...] = (
    ...
    Spec("WOVRA_ORG_WATERMARK", "水位 tok", "ctx", "int", "100000",
         lo=1000, hi=100000000, hint="到线触发结算/分裂/整理；下一轮生效"),
    Spec("WOVRA_FOLD_TARGET", "整理目标（水位比例）", "ctx", "float", "0.6",
         lo=0.05, hi=1.0, hint="整理到水位这个比例之下，一次到位"),
    Spec("WOVRA_COMPRESS_THRESHOLD", "窗口保底阈值", "ctx", "float", "0.8",
         lo=0.05, hi=1.0),
    Spec("WOVRA_MAX_TURNS", "单段最大步数（续跑重新给一段）", "ctx", "int", "200",
         lo=1, hi=10000),
