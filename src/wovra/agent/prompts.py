...
_SPLIT_INSTRUCTIONS = (
    ...
    "   **描述只写给顶层节点**（会成为子 agent 的那几个，每个 ≤60 字，写清\n"
    "   「干什么+产出什么」与边界）；**其余节点（分组/叶子）一概不写描述**——\n"
    "   缺描述由 Runtime 机械兜底（名字+文件清单），不会出现「无描述」。\n"
    "   单文件单轮的**叶子仍然必须有（一个活性文件一个叶子、`file` 填编号）**，\n"
    "   名字也可省（=用文件名）。产物越短越不容易被端点输出预算截断（截断是\n"
    "   分裂失败的头号来源：实测一次 13,523 字符的产物整批作废）。**省掉叶子\n"
    "   ＝这批结构树不完整**。\n"
    "   要写描述的节点**写满三件**——它是跨 agent 唯一的公共信息，\n"
            ...
            "properties": {
                "domains": {
                    ...
                    "description": (
                        ...
                        "分裂与否的判断。**填 [硬数据] 里的编号（`L00`、`L01`…），"
                        "不要写路径**——编号与文件的对应由 Runtime 机械翻译，"
                        "路径拼写问题在源头消失（写路径也能认，但要抄全、别加"
                        "反引号；抄错前缀会被归一到最近的节点）"
                    ),
                    "items": {
                        "type": "object",
