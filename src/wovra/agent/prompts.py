...
_SPLIT_INSTRUCTIONS = (
    ...
    "2. 文件域聚合：把 **LIVE 文件**（被写过、现在还在的；硬数据里状态 live）\n"
    "   按**功能语义**做相关性合并成架构——小域并成大域，叶子是**文件块**\n"
    "   （一个文件一个块）。域名 + 描述 + **files（具体文件清单）** + 目标 +\n"
    "   + 约束 + 块归属。**不是按路径**！路径只是线索：同目录可属不同域、\n"
    "   不同目录可属同域。非 LIVE（被取代的前史/只读/已删）**不是分裂单元**：\n"
    "   写进所属域的 history_files，挂在最相关 LIVE 文件下面当历史，\n"
    "   同一大类下有多条工作线时往下分层（parent 表达子域）。\n"
    "3. 分裂判定：看**架构最上层（顶层）的节点数**——\n"
    "   * **第一次分裂**：顶层 1 个节点**也裂**（裂 1 个子 agent，把全部文件活\n"
    "     搬进它，主 agent 只剩闲聊 + 环境块）；2 个及以上就一域一个子 agent。\n"
    "   * **子 agent 再分裂**：顶层只有 1 个节点就**往下一层看**，直到出现\n"
    "     ≥2 个节点；**最细到一个 LIVE 文件为止**——单个 LIVE 文件可以自成一域，\n"
    "     但不能再往下拆到块（LIVE 下面的块是历史交互，不可单独分裂）。\n"
    "   * 主 agent 的残留（闲聊/独立思想/环境块）**不算节点**，不参与计数。\n"
    "   * 候选单元数不得超过硬数据的 LIVE 文件数（硬数据里给出）。\n"
    "**域归属完整性（强制）**：每个 LIVE 文件必须**恰好**落在一个域的 files\n"
    "里；分块地图里的每个块也必须有归宿（某域的 block_ids，或 unassigned\n"
    "归主 agent）。**禁止重叠**（一个文件出现在两个域里）——Runtime 会机械\n"
    "校验并**拒收**有重叠或有遗漏的产物（2026-09-12 用户口径：这类根基错误\n"
    "直接停，不许静默兜底）。\n"
    ...
    "域描述（description）必填，而且必须写满**三件**（2026-09-13 用户口径："
    "「职责说明书现在太松散了」——它是**跨 agent 唯一的公共信息**，主 agent 只凭"
    "它路由，写松了就会出现「同一话题换个人答」）：\n"
    "   ①**干什么 + 产出什么**（点出关键文件/产物名）；\n"
    "   ②**边界**：与最容易混的那个域怎么分——什么归我、**什么不归我**"
    "（写法如「外部方案调研在 B，本域只做自研实现与加固」）；\n"
    "   ③**什么信号落到我这里**：用户提到哪些文件/名字、问哪一类问题时该找我"
    "（如「问『能不能做到/怎么修』找我；问『外面有什么现成的』找 B」）。\n"
    "**输出精简（强制）**：块的归属若与其文件一致（块的文件 ∈ 某域\n"
    "files），**不必**写进 block_ids——Runtime 按文件归属自动归入；\n"
    "block_ids 只列需要显式声明的（纯聊天块归属、跨域块、与文件归属不符\n"
    "的例外）。submit_domains 参数必须在单次响应内完整输出——超长截断\n"
    "会令整个分析作废（实测）。\n"
    "4. 逐层分裂（正式机制，不是可选项）：硬数据里若某个域**已达它自己\n"
    "   的水位**，就该在它内部再裂一层——子域 parent 写该域名，一直到\n"
    "   「单个 LIVE 文件」为止（一个 LIVE 文件可以自成一域，但不能再拆到块）。\n"
    "   **永远只有两级**（2026-09-12 用户口径）：一个域分裂之后**它自己就没了**，\n"
    "   被它的子域取代——注册表里永远是「主 agent + 一排平级子域」。路径 ID\n"
    "   由 Runtime 机械生成：主 agent 分裂出的域取 A、B、C…；A 再分裂则产物\n"
    "   是 A-1、A-2…（名字留血缘，层级仍平级，A 不再存在）。\n"
    "   没到水位的域**不要拆**：拆分本身也有成本，只有隔离收益超过拆分\n"
    "   成本时才值得拆（经济判据由 Runtime 按机械算式记账，你只做语义\n"
    "   判断：这一摊活是否真已分成互不相干的两条线）。\n"
        ...
        "parameters": {
            ...
            "properties": {
                "domains": {
                    "type": "array",
                    "description": (
                        "文件域：对**LIVE 文件**（被写过、现在还在的）按**功能"
                        "语义**做相关性合并成架构（不是按路径！路径只是线索——"
                        "同目录可属不同域、不同目录可属同域）。叶子是**文件块**"
                        "（一个文件一个块，块里含该文件全部轮次的交互）。"
                        "层级用 parent 表达（子域 parent=父域 name，顶层留空）——"
                        "同一大类下有多条工作线时要往下分层，不能只给顶层。"
                        "每个域给 **files（具体文件清单）**；只读/被删/被取代的"
                        "写进 history_files，作为历史挂在本域下面，**不单独分裂**。"
                        "被后续重写/取代的早期版本同理（superseded 前史）"
                    ),
                    "items": {
                        "type": "object",
                        "properties": {
                            ...
                            "files": {
                                ...
                                "description": (
                                    "该域现时拥有的**具体文件清单**（写全路径，"
                                    "一个文件只能出现在一个域里）。**分裂单元"
                                    "只有 LIVE 文件**（被写过、现在还在的）；"
                                    "只读过的与被删/被取代的文件写进 "
                                    "history_files，挂在最相关的 LIVE 文件下"
                                ),
                            },
                            "file_domains": {
                                "type": "array",
                                ...
                                "description": (
                                    "【旧字段，仅为兼容】路径前缀。新分析请用 "
                                    "files 给具体文件——按路径划分会造成域之间"
                                    "重叠（实测 `src/` 与 `src/wovra/` 会同时命中"
                                    "一个文件），Runtime 现在会拒收有重叠的产物"
                                ),
                            },
                            "history_files": {
                                "type": "array",
                                "items": {"type": "string"},
                                "description": (
                                    "挂在本域下面的**历史文件**（被重构替代/只读/"
                                    "被删的）——它们不是分裂单元，只是这条工作线"
