...
_SPLIT_INSTRUCTIONS = (
    ...
    "3. 分裂判定：顶层节点 = 各域 + **主 agent 残留桶**（unassigned/纯聊\n"
    "   天块）并列。残留桶按硬数据给出的\"纯对话块内容占比\"计：占比 ≤15%\n"
    "   （≈15K @ 100K 水位）→ 不构成独立节点（并入主 agent，不拆出）；\n"
    "   >15% → 与域并列计数。找**能形成有效分裂的最浅层**——有效顶层\n"
    "   节点 ≥2 个就用顶层；顶层只有 1 个节点但它有 ≥2 个子节点，用该\n"
    "   子层；全链单子（所有内容本质同属一条工作线）则不可分。候选单元\n"
    "   数不得超过硬数据的活性文件数上限。\n"
    ...
    "file_domains），**不必**写进 block_ids——Runtime 按文件归属自动归入；\n"
    "block_ids 只列需要显式声明的（纯聊天块归属、跨域块、与文件归属不符\n"
    "的例外）。submit_domains 参数必须在单次响应内完整输出——超长截断\n"
    "会令整个分析作废（实测）。\n"
    "4. 逐层分裂（正式机制，不是可选项）：硬数据里若某个域**已达它自己\n"
    "   的水位**，就该在它内部再裂一层——子域 parent 写该域名（如顶层域 A\n"
    "   下裂出子域），一直到「只操作单个文件为止」。路径 ID 由 Runtime\n"
    "   机械生成：顶层域取 A、B、C…，子域是 `A-1`、`A-2`…；管理关系\n"
    "   永远只有一层（主 agent 直管所有域，路径 ID 管职责，不是管理树）。\n"
