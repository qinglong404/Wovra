...
_SPLIT_INSTRUCTIONS = (
    "[分裂结构指令]\n"
    "你的**唯一任务**：把[硬数据]里的**活性文件**梳理成一棵**多层级结构树**，\n"
    "并把非 LIVE 文件、保底块、用户块挂到对应的节点上。\n"
    "**分工**（2026-09-14 用户口径）：你只负责**关系**——不相关的隔开、\n"
    "相关的归成一条线（谁和谁是一摊活、这块讲的是哪条线）；\n"
    "**归宿由代码算**——是否分裂、在哪一层分裂、每个节点/块最终归哪个\n"
    "agent，都由 Runtime 按你给出的结构机械决定。\n"
    "树形（叶子 = 单个活性文件），照项目的实际组织结构分层，该深就深：\n"
    "    项目架构\n"
    "    ├── 核心业务层\n"
    "    │   ├── 接口与控制器\n"
    "    │   │   ├── 路由文件\n"
    ...
    "            └── 测试文件\n"
    "                └── 用户服务.test.ts\n"
    "1. 内部节点 = 语义层/分组（填 name + description + parent），**不填 file**；\n"
    "   叶子 = **一个活性文件**（file 填[硬数据]的**编号**：`L00`、`L01`…。\n"
    "   **只写编号、不写路径**——编号与文件的对应关系由 Runtime 机械翻译，\n"
    "   路径拼写问题在源头消失；编号未知/把 H 编号填进叶子会被判错重来）。\n"
    "   节点名省了就用文件名。每个活性文件**恰好**一个叶子。\n"
    "2. 挂在节点上的三类附属物（不改归属，只是记录；挂哪层由语义决定，\n"
    "   通常挂到对应叶子，整条线共同的就挂上层节点）：\n"
    "   - history_files：非 LIVE 文件（被取代/只读/已删的旧版本）——填\n"
    "     [硬数据]非 LIVE 段的**编号**（`H00`…；只写编号），挂到相关叶子下，\n"
    "     **一个不漏**。它们虽然可能已不在磁盘上，但承载着早期工作的结论\n"
    "     （某次测试验了什么、结果如何），后面装配上下文要用；漏一个\n"
    "     Runtime 会拒收整批；\n"
    "   - chat_block_ids：保底块（讨论/决策这条工作线的纯聊天块，ID 逐字取自\n"
    "     [分块地图]，如 R14-B1、R19-24-B1）；\n"
    "   - user_block_ids：用户块（轮头之后的补充/修正发言，如 R25-B1）。\n"
    "3. 与任何工作线都无关的保底块/用户块 → unassigned（归主 agent）。\n"
    "   分块地图里的每个保底块/用户块都要有归宿：一个不落、不许重叠。\n"
    "   **块 ID 逐字取自[分块地图]**（含合并组编号如 R19-24-B1）——写了地图里\n"
    "   不存在的 ID 会被 Runtime 判错并整批重来（带诊断重发一次）。\n"
    "4. 结构稳定：同一批数据两次分析应给出同构的树（数量与层级一致）。\n"
    "   节点描述（description）写满**三件**——它是跨 agent 唯一的公共信息，\n"
    "   主 agent 只凭它路由（2026-09-13 用户口径：写松了会出现「同一话题换\n"
    "   个人答」）：\n"
    "   ①**干什么 + 产出什么**（点出关键文件/产物名）；\n"
    "   ②**边界**：与最容易混的那个节点怎么分——什么归我、**什么不归我**\n"
    "     （如「外部方案调研在 B，本节点只做自研实现与加固」）；\n"
    "   ③**什么信号落到我这里**：用户提到哪些文件/名字、问哪一类问题时该找\n"
    "     我（如「问『能不能做到/怎么修』找我；问『外面有什么现成的』找 B」）。\n"
    "**输出精简**：文件块按叶子 file 自动归属，不必逐块列 block_ids——\n"
    "block_ids 只列例外（跨节点块）。submit_domains 参数必须在单次响应内\n"
    "完整输出——超长截断会令整个分析作废（实测）。\n"
        ...
        "parameters": {
            ...
            "properties": {
                "domains": {
                    "type": "array",
                    "description": (
                        "现状结构树：把**活性文件**（被写过、现在还在的）按**功能"
                        "语义**聚合成树（不是按路径！路径只是线索——同目录可属"
                        "不同节点、不同目录可属同一节点）。叶子是**文件**。"
                        "层级用 parent 表达（子节点 parent=父节点 name，顶层留空）——"
                        "同一大类下有多条工作线时要往下分层，不能只给顶层。"
                        "**是否分裂由代码按这棵树决定**：你只给出结构，不做"
                        "分裂与否的判断。files 的路径**逐字取自[硬数据]的活性文件"
                        "清单**（不要加目录前缀、不要改写法）"
                    ),
                    "items": {
                        "type": "object",
                        "properties": {
                            ...
                            "file": {
                                ...
                                "description": (
                                    "**叶子节点专用**：这个节点对应的**一个活性"
                                    "文件**，填[硬数据]里的**编号**（`L00`、`L01`…"
                                    "——硬数据每行是 `Lxx = 路径`）。**只写编号，"
                                    "不要写路径**；编号未知、或把非 LIVE 编号（H…）"
                                    "填到这里，Runtime 会判错并整批重来。内部节点"
                                    "（分组/层级）不填本字段"
                                ),
                            },
                            "chat_block_ids": {
                            ...
                            "history_files": {
                                ...
                                "items": {"type": "string"},
                                "description": (
                                    "挂在本节点下面的**非 LIVE 文件**（被取代/"
                                    "只读/已删的旧版本）——填[硬数据]非 LIVE 段的"
                                    "**编号**（`H00`、`H01`…，每行是 `Hxx = 路径`），"
                                    "只写编号。**必须一个不漏**（漏了 Runtime 拒收"
                                    "整批）；它们承载早期工作的结论，后面装配"
                                    "上下文要用"
                                ),
                            },
