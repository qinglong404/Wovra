...
> **没有任何"预约下一轮"的入口**（2026-09-12 用户口径）：`switch_view` 工具
> 已删——agent 不能决定下一轮归谁。规则只产出**起手建议**（`route_hint`），
> 真正定归属的是"这一轮实际是谁在做"（轮上的 `active_view`）。
> `route(explicit=…)` 那个参数只服务**历史数据**（老轮的 `route_explicit`）。

1. **历史显式意志**（老数据才有）：那一轮当初被预约给谁，补判时沿用；
...
路由准确率不必 99%：接手方发现"这不是我的活"时，用 `route_to` 在本回合内
把它转给更相关方（跨域参考自己 read_file 解决，见身份卡的接活判据）。只付
一跳的代价——故规则宁可精，不必贪全。
"""
import os
...
def identity_card(name: str, registry: Optional[Iterable[dict]]) -> list[str]:
    ...
    **主 agent 先判**（2026-09-12 修正）：注册表里恒有 `A` 条目，若先查
    ...
    if str(name) == MAIN_AGENT_ID:
        return [
            # **主 agent 只路由，不干活**（2026-09-17 用户口径："主 agent 不可以干任何活，
            # 除了路由，不然会污染上下文"）。旧文案写的是"路由器 + 兜底执行者"、
            # "落不到 → 才自己做"——与现行机制相反（2026-09-18 用户实测：模型据此对外
            # 解释机制时说的是旧模型）。
            f"[当前身份] {MAIN_AGENT_ID}（主agent）：**只做路由，不干活**。"
            "收到用户消息，看它与上一条的关系，再对职责表问一句"
            "「这活落在谁的域里」：\n"
            ...
            "它在本回合内直接接手，结果直接给用户；\n"
            "2. **读也算活**：要动某个域名下的文件（哪怕只是读一眼）也走 route_to "
            "交给主人——主人手里往往已经有那份内容，还能省一次读；\n"
            "3. **没有主的新活** → `route_to(agent=\"new\")` 就地起一个 agent 接它，"
            "别自己揽下来；\n"
            "4. **只有一类留给你**：完全不碰文件的对话（寒暄、身份确认、"
            "关于机制本身的问答）。这类没有交付物、也不该有人接手，你直接答。\n"
            "一句话跨两摊活：挑**交付物主要落在哪个域**就转给谁；跨域的参考"
            "由它自己去读（文件系统是共享的，读文件不需要归属）。转错了也不"
            "致命——对方更懂那摊事，它会就地做完或当场说明该找谁，代价只是一"
            "跳；自己硬做才是贵的。"
        ]
    ...
    lines.append(
        # V4：上下文是**全体共享的一份公共历史**（2026-09-17 "取消重组"），不再按职责域
        # 各切一份。旧文案写的是"你只看到属于自己这摊活的历史——其他职责域的内容不在你的
        # 上下文里"，与现行机制相反（模型据此对外解释机制时说的就是旧模型）。
        "上下文纪律：历史是**全体共享的一份**——所有 agent 看到同一份公共历史，"
        "按职责域分的只有**文件归属**（你文件域内的文件你可以读写改删，别人的只读）。"
        "需要现状直接 read_file/search_files 去读；要别人的判断用 consult 问属主域，"
        "只是知会一声用 notify。"
    )
    lines.append(
        "**接活判据（只看交付物落在哪，不看要不要读别人的文件）**：\n"
        "1. 这一轮的**产出/改动**主要落在我的所有权文件域里 → **就地干**，"
        "哪怕要读、甚至要改好几个别的域的文件。**新需求尤其如此**——新东西"
        "本来就可能要参考别处的现状、先动别处一小块；你把它干出来，体量涨到"
        "水位自然会被分裂成合适的域，不要为了「纯洁」先转走或退回；\n"
        "2. 产出**整个**落在别人的域里（我做的只是转手，一行都不写在我这儿）"
        "→ 一上手就 route_to 转给更相关的 agent（本回合内生效，它直接接手）；"
        "不要先干一半再退回——半途退货是最贵的纠错；\n"
        "3. 拿不准 → **就地干**。转走一轮的代价是一整轮时间，而留错地方的活"
        "只是让体量记录偏了一点，分裂时会被纠正。"
    )
    return lines
