...
* **网页检索走专业 API**：`Wovra_Tavily`、`Wovra_Serper`、`Wovra_Exa`、`Wovra_Bocha`、
  `Wovra_SerpAPI`、`Wovra_Firecrawl`、`Wovra_TinyFish` 填哪几家都行（各家惯用名 `*_API_KEY`
  与通用的 `Wovra_SEARCH_KEY` 也认）。每次检索**随机**挑一家先试，失败就换下一家——把调用摊到各家
  ...
  改问它要干净的 Markdown。）
* **按需的云浏览器**：`web_automate(url, goal)` 把一句自然语言目标交给云浏览器（TinyFish
  Agent API，SSE 流式）：执行轨迹进账本、结果作为文本返回，**停止本轮会直接断开这条流**
  而不是干等。它对付的是"必须真交互才拿得到东西"的页面（登录墙、筛选、SPA 渲染的数据）。
  它**按步计费**，所以永远不做自动兜底；而同一家的 **Search 与 Fetch 是免费的**——正因如此
  `web_fetch` 抽不到正文时先问 TinyFish Fetch 要渲染后的 Markdown，付费的 Firecrawl 排在后面。
...
* [x] 最小智能体运行时（managed **33** / baseline **23** 个工具：文件 / 命令 / 后台任务 / 网络检索（搜索·抓取·**云浏览器自动化**）/ **眼睛（截屏·看图·页面文本）** / 交互确认 / 计划账本 / 历史展开 / 路由 / 单向通知与双向咨询 / 会合 / 职责更新 / **整理与分裂的提交口**）
